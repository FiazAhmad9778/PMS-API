...
public class GetWardDropdownFromPMSQuery : IRequest<ApplicationResult<List<WardResponseDto>>>
{
  [System.ComponentModel.DataAnnotations.Required]
  public long OrganizationId { get; set; }
}

public class GetWardDropdownFromPMSQueryHandler : RequestHandlerBase<GetWardDropdownFromPMSQuery, ApplicationResult<List<WardResponseDto>>>
{
  private readonly IConfiguration _configuration;
  private readonly string _connectionString;
  private readonly string _databaseName;

  public GetWardDropdownFromPMSQueryHandler(
  ...
  protected override async Task<ApplicationResult<List<WardResponseDto>>> HandleRequest(GetWardDropdownFromPMSQuery request, CancellationToken cancellationToken)
  {
    try
    {
        ...
        using (var command = new SqlCommand(query, connection))
        {
          ...
          using (var reader = await command.ExecuteReaderAsync(cancellationToken))
          {
            while (await reader.ReadAsync(cancellationToken))
            {
              ...
              {
                var externalId = reader.IsDBNull("ExternalId") ? 0 : reader.GetInt32("ExternalId");
    ...
    {
      Logger.LogError(ex, "Error fetching ward dropdown from Pharmacy database");
