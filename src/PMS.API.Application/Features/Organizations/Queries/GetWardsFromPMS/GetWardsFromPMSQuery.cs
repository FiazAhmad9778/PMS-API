...
public class GetWardsFromPMSQuery : IRequest<ApplicationResult<List<WardResponseDto>>>
{
  public long OrganizationId { get; set; }
  public string? SearchKeyword { get; set; }
}

public class GetWardsFromPMSQueryHandler : RequestHandlerBase<GetWardsFromPMSQuery, ApplicationResult<List<WardResponseDto>>>
{
  private readonly IConfiguration _configuration;
  private readonly string _connectionString;
  private readonly string _databaseName;

  public GetWardsFromPMSQueryHandler(
    IConfiguration configuration,
    IServiceProvider serviceProvider,
    ILogger<GetWardsFromPMSQueryHandler> logger) : base(serviceProvider, logger)
  {
  ...
  protected override async Task<ApplicationResult<List<WardResponseDto>>> HandleRequest(GetWardsFromPMSQuery request, CancellationToken cancellationToken)
  {
    try
    {
        ...
        using (var command = new SqlCommand(query, connection))
        {
          ...
          using (var reader = await command.ExecuteReaderAsync(cancellationToken))
          {
            while (await reader.ReadAsync(cancellationToken))
            {
              ...
              {
                var externalId = reader.IsDBNull("ExternalId") ? 0 : reader.GetInt64("ExternalId");
