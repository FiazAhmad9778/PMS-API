...
public class UpdateUserSignatureCommand : IRequest<ApplicationResult<long>>
{
  public string? SignatureBase64 { get; set; }
}

public class UpdateUserSignatureHandler : RequestHandlerBase<UpdateUserSignatureCommand, ApplicationResult<long>>
{
  private readonly IUserRepository _UserRepository;
  private readonly IIdentityService _identityService;
  public UpdateUserSignatureHandler(
    IUserRepository UserRepository,
    IServiceProvider serviceProvider,
    ILogger<UpdateUserHandler> logger,
    IIdentityService identityService) : base(serviceProvider, logger)
  ...
  protected override async Task<ApplicationResult<long>> HandleRequest(UpdateUserSignatureCommand request, CancellationToken cancellationToken)
  {
    var existingUser = await _UserRepository.FirstAsync(x => x.Id == _currentUser.Id);
    if (existingUser == null) return ApplicationResult<long>.Error("User not found!");
